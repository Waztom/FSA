...
```{r}
world_data   <- read.csv("si_be.csv", header = TRUE, sep = ",")
world_data   <- world_data %>% rename(reporter=destin) %>% rename(partner=origin)

period_list               <- list() #List of months
beer_net_list                  <- list() #Find network objects by index eg. net_list[[24]]
...
for (i in world_monthly_data){
  ...
  beer_net_list      <- list.append(beer_net_list, net)
...
model.1c <- btergm(beer_net_list ~ edges + transitiveties + isolates + mutual + odegree1.5 + idegree1.5 + memory(type = "stability"), R = 100, parallel = "snow",  ncpus = 2)
...
#Save model
saveRDS(model.1c, "beer_network_model_si.rds")

#Save net_list
saveRDS(beer_net_list, "beer_net_list.rds")
