...
model.1a <- btergm(net_list ~ edges + mutual + transitiveties  + nodecov("nett_trade") + nodecov("trade_flux") + nodecov("nett_weight") + nodeicov("trade_value") + nodeicov("trade_weight_value") + timecov(transform = function(t) t), R = 25, parallel = "snow",  ncpus = 5)

#See parameter estimates etc with 95% CI - how do we interpret these values?
summary(model.1a, level = 0.95)

#Goodness of fit test
gof.1a <- gof(model.1a, nsim = 5, statistics = c(edgebetweenness.modularity,esp,nsp,ideg,odeg, edgebetweenness.pr))
plot(gof.1a)
dev.off()
