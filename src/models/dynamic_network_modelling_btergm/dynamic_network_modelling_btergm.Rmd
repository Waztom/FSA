...
model.1c <- btergm(net_list ~ edges + mutual + transitiveties, R = 5, parallel = "snow",  ncpus = 2)
...
model.1d <- btergm(net_list[1:23] ~ edges + mutual + transitiveties, R = 5, parallel = "snow",  ncpus = 2)
#2. Test GOF by comaring simulated network with actual network
gof.1d <- gof(model.1d, nsim = 10, target = net_list[[24]], 
formula = net_list[23:24] ~ edges + mutual + transitiveties, coef = coef(model.1d),statistics = c(esp,nsp,ideg,odeg))
...
interpret(model.1a, type = "tie", i = 193, j = 174, t = 15)
```