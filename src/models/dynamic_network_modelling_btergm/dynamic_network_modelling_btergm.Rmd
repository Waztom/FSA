...
model.1a <- btergm(net_list ~ edges + mutual + transitiveties + nodecov("nett_trade") + nodecov("trade_flux") +nodecov("nett_weight") , R = 50, parallel = "snow",  ncpus = 2)

#See parameter estimates etc with 95% CI - how do we interpret these values?
summary(model.1a, level = 0.95)

#Goodness of fit test
gof.1a <- gof(model.1a, nsim = 5, statistics = c(esp, geodesic, deg))
plot(gof.1a)
dev.off()

#Try covariate model with temporal component - how to make list index a date? 
model.1b <- btergm(net_list ~ edges + mutual + transitiveties + nodecov("nett_trade") + nodecov("trade_flux") +nodecov("nett_weight") + timecov(transform = function(t) t), R = 50, parallel = "snow",  ncpus = 2)
