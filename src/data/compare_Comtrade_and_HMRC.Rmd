...
eu_list <- c("BE","BG","CZ","DK","DE","EE","IE","EL","ES","FR","HR","IT","CY","LV","LT","LU","HU","MT","NL","AT","PL","PT","RO","SI","SK","FI","SE","UK")
```
...
#tmp1 <- tmp %>% filter(str_sub(smk_comcode,1,str_length(com_id)) == com_id)
tmp2 <- tmp[str_detect(tmp$smk_comcode,paste('^',com_id,sep='')),]
#Remove crazy year
current_year <- 2018
tmp2 <- tmp2 %>% filter(as.numeric(smk_period_reference)<100*(current_year+1))
...
tmp2 <- tmp2 %>% mutate(period = paste(str_sub(period_tmp,4,7),str_sub(period_tmp,1,2),sep='')) %>% select(-period_tmp)
...
3. Do the plots comparing both databases
3.1 Net weight in kg
```{r}
