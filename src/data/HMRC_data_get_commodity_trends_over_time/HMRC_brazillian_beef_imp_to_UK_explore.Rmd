...
author: "Warren Thompson & Phoebe MacDonald"
output:
  pdf_document:
...
model_data_pretbl    <- HMRC_EU_import_food_data %>% 
...
%>% 
                    ...
                    filter(Date!="0000000") %>%  
                    mutate(abs(Quantity_1)  &  abs(Value)) %>% unique()
```
...
#Aggregate data into periods of months
```{r}
model_data_group_1617 <- model_data_clean_1617 %>% group_by(Date) %>%                          
                                        summarise(Quantity_1 = sum(Quantity_1),                          
                                                  Value = sum(Value), 
                                                  Cost_per_Kg = mean(Cost_per_Kg))
```
