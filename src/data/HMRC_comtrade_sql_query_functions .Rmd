...
# Function to access selected sql database. Enter database == HMRC or comtrade to access the required db and enter df_description for HMRC df example arrivals
```{r}
HMRC_sql_access <- function(df_description){

# Convert function inputs to strings
df_description <- as.character(substitute(df_description))
...
sql_db_query <- paste("SELECT * FROM",df_description,"WHERE smk_comcode ~ '^[0-2]'")
...
```{r}
comtrade_sql_access <- function(from_period,to_period){
