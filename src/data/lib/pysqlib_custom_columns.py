...
def comtrade_sql_request(reporter_name = 'United Kingdom', partner_name = 'Brazil', commodity_name='Meat of bovine', requested_columns = ['partner', 'trade_flow_code','netweight_kg', 'trade_value_usd', 'period', 'commodity_codes']):
    ...
    query = sql.SQL("SELECT {query_columns} FROM comtrade WHERE "\
...
#                "AND reporter_code = %(reporter)s", {'query_columns': sql.SQL(', ').join(sql.Identifier(n) for n in requested_columns), 'partner': partner_code, 'comcodes': com_codes, 'reporter': reporter_code}))
