...
df2_analysed <- df2 %>% 
    time_decompose(ratio, method = "stl") %>% anomalize(remainder, alpha = 0.5, method = "iqr") %>% filter(period_date == (all_months[date1])) %>% filter(anomaly == "Yes") %>% select(node, remainder, remainder_l1, remainder_l2)
...
a_total <- rbind(a_high, a_low) %>% mutate(Deviation = abs(remainder_new)) %>% arrange(desc(Deviation)) %>% select(node, Deviation) %>% rename(Country = node)
