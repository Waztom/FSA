...
```{r}
ggplot(mydata) + geom_point(aes(x=total_import/1e6,y=ratio,
                       color=as.integer(degree_val),size=as.integer(bet_val)+1),alpha=0.7) +
 #                      geom_text(aes(x=total_import/1e6,y=ratio,label=nodes$label)) +
                       scale_colour_gradientn(colours=rainbow(4)) +
  labs(x="Total Imports (thousands of tons)",y="(Imp - Exp) / (Imp + Exp)",
