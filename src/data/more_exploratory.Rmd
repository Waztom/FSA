...
maxval <- check %>% summarize(maxval = max(imbalance-1,na.rm=TRUE))
maxval <- maxval[1,1]
minval <- check %>% summarize(minval = min(imbalance-1,na.rm=TRUE))
minval <- minval[1,1]
rscore <- maxval - minval

check <- check %>% mutate(wei1 = (imbalance-1)/rscore) %>%
                   mutate(wei2 = (trade_weight-minval)/rtrade) %>%
                   mutate(weighted_imbalance = wei1/wei2)

best_origin <- check %>% group_by(origin) %>%
  summarize(score = mean(weighted_imbalance,na.rm=TRUE)) %>% filter(!is.na(score))
