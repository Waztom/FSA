...
anomaly_summary <-Chicken_data_simple %>% 
            ...
            anomalize(remainder, alpha = 1, method = "iqr") 
...
anomaly_summary <-Chicken_data_simple %>% 
            time_decompose(ratio, method = "stl") %>% 
            anomalize(remainder, alpha = 5, method = "gesd") 
...
Chicken_data_simple <- Chicken_data_normalised  %>%
  select(period_date, ratio, node) %>% 
   filter(node %in% selected_countries_new) %>% ungroup %>% group_by(node)
...
```{r}
Chicken_data_normalised %>% group_by(node) %>% summarise(count = n()) %>% filter(count > 10) %>% ungroup -> above_ten
