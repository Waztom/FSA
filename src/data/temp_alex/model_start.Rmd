...
```{r}
#pathname <- "/Users/Caro_i_Alex/FSA/src/data/Chicken/"
#filename <- "world_chicken_data.csv"
pathname <- "/Users/Caro_i_Alex/FSA/src/data/"
...
```{r}
pick_a_month   <- 201402
pick_a_country <- "Canada"
test <- all_info %>% filter(period == pick_a_month) %>%
                     filter(node == pick_a_country) %>%
                     select(overall_flux,deg_in_wei, deg_out_wei, degree_net, bet_val, tri_no, eigen_val, net_group)
