...
```{r}
mydata <- all_info %>% select(month, deg_out_wei, deg_in_wei, tot_in_wei_n, tot_out_wei_n, ratio, bet_val, overall_flux, eigen_val)
...
for (i in 2:15) wss[i] <- sum(kmeans(mydata_s, centers=i)$withinss)
...
results   <- data.frame(mydata,   fit$cluster, all_info$node) %>% rename(cluster = fit.cluster) %>% rename(node = all_info.node)
```
