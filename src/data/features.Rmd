...
```{r}
tmp_dat <- all_info %>% filter(period == 201401) %>% select(node, degree_val, degree_net, ratio, bet_val, overall_flux, deg_in_wei, deg_out_wei)
...
```{r}
linmodel <- all_info %>% filter(period == 201401) %>% select(overall_flux,degree_val,bet_val,ratio,tri_no,eigen_val)
