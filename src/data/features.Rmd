...
```{r}
mydata <- all_info %>% select(month, degree_net, ratio, bet_val, overall_flux)
mydata_s <- scale(mydata) # Data must be scaled!!
```

```{r}
ggplot(all_info) + geom_point(aes(x=month,y=degree_net,color=ratio,size=overall_flux)) + scale_colour_gradientn(colours=rainbow(4))
```
