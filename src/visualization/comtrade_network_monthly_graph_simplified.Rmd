...
#I want to remove interactions between children
#I want to colour code depending on whether its an import or export
```
...
communities <- edge.betweenness.community(h) 
sizes(communities)
...
visNetwork(nodes_groups, edges) %>% 
  ...
  visNodes(font=list(size=20), shadow = TRUE) %>%
#shows 1st degree children and parents
  visOptions(highlightNearest = list(enabled = TRUE, degree=list(to=1, from=1), algorithm="hierarchical"), nodesIdSelection = TRUE, clickToUse=TRUE) %>%
  visInteraction(navigationButtons = TRUE) %>%
