...
```{r}
total_imports<- world_cucumber_data_graph_exlworldEU_simple %>% group_by (partner) %>% summarize(total_import = sum(trade_value_usd)) %>% rename(label = partner)

total_exports <- world_cucumber_data_graph_exlworldEU_simple %>% group_by (reporter) %>% summarize(total_export = sum(trade_value_usd)) %>% rename(label = reporter)


nodes_nets <- nodes_groups %>% left_join(total_imports, by = "label") %>% left_join(total_exports, by = "label")

nodes_nets_clean <- nodes_nets %>% replace_na(list(total_import=0, total_export=0))
