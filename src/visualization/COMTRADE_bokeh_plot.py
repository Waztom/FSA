...
trade_data.set_index(trade_data['partner']+' -> '+trade_data['importer'])

trade_data.sort_values('value', ascending=False).plot.bar()
        
chord_from_df = Chord(trade_data, source="partner", target="importer", value="value")
output_file('figures/chord-diagram-bokeh.html', mode="inline")
show(chord_from_df)
