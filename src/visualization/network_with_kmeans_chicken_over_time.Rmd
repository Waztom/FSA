...
world_chicken_monthly_data <- get_Comtrade_monthly_data(world_chicken_data_csv)
```
. 
#Prepare data
```{r}
world_chicken_data_graph <-for (i in world_monthly_data){world_monthly_data %>% filter(trade_flow=="Imports") %>% select(period,reporter,partner,trade_value_usd) %>% mutate(period = paste0(period,"01")) %>% mutate(period = as.Date(period, "%Y%m%d"))}
```
...
```
#using betweenness/200 to try to regulate it a bit (at the moment the clusters more or less depend on betweenness)
```{r}
