...
trade = pd.read_pickle('./latest_network.pickle')

trade_all = trade.unstack()

quantile = 0.5
trade_quantile = trade_all.quantile(q=quantile)
    ...
    for partner in trade.index:
        if not np.isnan(trade.get_value(partner, importer)):
            if trade.get_value(partner, importer) > trade_quantile:
                ...
                G.add_edge(partner, importer, weight= np.log10(trade.get_value(partner, importer)), alpha=0.4)
#            else:
#                print(partner+ ' -> '+importer+' : '+str(trade.get_value(partner, importer)))
...
norm_weights = matplotlib.colors.LogNorm(vmin=weights.min(), vmax=weights.max())

f, ax = plt.subplots(1,figsize=(2*13.7, 2*9), dpi=600)
plt.subplots_adjust(left=0.12, right=0.96, top=0.95, bottom=0.15)
ax.set_title('Vanilla importer network to United Kingdom')

#pos = nx.nx_agraph.graphviz_layout(G, prog='twopi')
#pos = nx.nx_agraph.graphviz_layout(G, prog='neato')
pos = nx.fruchterman_reingold_layout(G, weight=norm_weights, scale=5)
...
# Creates an initial colourlist where all edges have same colour
edge_colours=['skyblue']*len(edges)

# Change colour of edges connected to UK
edgecount=0
for edge in edges:
    if 'United Kingdom' in edge:
        edge_colours[edgecount] = 'Tomato'
    edgecount = edgecount +1

nx.draw_networkx_edges(G, pos, width=3*norm_weights( weights).data, #0.5
...
#plt.colorbar()
plt.axis('off')
plt.savefig('vanilla_net_'+str(quantile)+'.pdf', dpi=600)
