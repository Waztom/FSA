...
```{r}
world_data   <- read.csv("world.csv", header = TRUE, sep = ",")
```
...
#Prepare data
```{r}
world_data_graph <- world_data %>% filter(trade_flow=="Imports") %>% select(period,reporter,partner,trade_value_usd) %>% mutate(period = paste0(period,"01")) %>% mutate(period = as.Date(period, "%Y%m%d"))
```


#cut out world, EU-27 and othjer groupings
```{r}
world_data_graph <- world_data_graph %>% filter(partner != "World") %>% filter(partner != "EU-27")%>% filter(partner != "Other Asia, nes") %>% filter(partner != "Other Europe, nes") %>% filter(partner != "Area, nes")
world_data_graph <- world_data_graph %>% filter(reporter != "World") %>% filter(reporter != "EU-27")%>% filter(reporter != "Other Asia, nes") %>% filter(reporter != "Other Europe, nes") %>% filter(reporter != "Area, nes")
```

#Create nodes 
```{r}
imports_from <- world_data_graph %>% select(reporter) %>% rename(label = reporter) %>% distinct(label)
imports_to <- world_data_graph %>% select(partner) %>% rename(label = partner) %>% distinct(label)
...
#Create edges table 
```{r}
edges <-  world_data_graph %>% left_join(nodes, by = c("reporter" = "label")) %>% rename(to = id) %>% left_join(nodes, by = c("partner"  = "label")) %>% rename(from = id)
