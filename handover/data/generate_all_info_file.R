...
#    eigen   <- eigen_centrality(g, weights = NA) #Unweighted eigenvalue
    ...
    for (cur_country in all_country){
      ...
      dat2[i] <- betweeness[this_one]
      dat3[i] <- triangles[this_one]
      dat4[i] <- eigen$vector[this_one]
...
dfs <- list("links_tot" = dat1, "betweeness" = dat2, "triangles" = dat3, "eigen" = dat4, "net_group" = dat5, "node" = dat6, "period" = dat7)
