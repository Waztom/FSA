get_trade_attributes_test <- function(df){
  ...
  world_imports_to    <- df %>% filter(trade_flow == "Imports") %>% select(reporter,trade_value_usd) %>% group_by(reporter) %>% summarise_at(.vars = vars(trade_value_usd), .funs = sum) %>% rename(label=reporter)
  
  world_imports_from  <- df %>% filter(trade_flow == "Imports") %>% select(partner,trade_value_usd) %>% group_by(partner) %>% summarise_at(.vars = vars(trade_value_usd), .funs = sum) %>% rename(label=partner)
